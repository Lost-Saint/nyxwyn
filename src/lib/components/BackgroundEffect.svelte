...
<div aria-hidden="true" class="fullscreen-overlay" bind:this={refDiv}></div>

<svg class="background-boids" viewBox="0 0 {width ?? 0} {height ?? 0}">
	<defs>
		{#each boids as boid, i (boid)}
			<radialGradient id="boidGradient{i}" cx="50%" cy="50%" r="50%" fx="50%" fy="50%">
				<stop offset="0%" stop-color="hsla({boid.hue}, 100%, 50%, 0.1)" />
				<stop offset="100%" stop-color="hsla({boid.hue}, 100%, 50%, 0)" />
			</radialGradient>
		{/each}
		...
		<radialGradient id="mouseGradient" cx="50%" cy="50%" r="50%" fx="50%" fy="50%">
			<stop
				offset="0%"
				stop-color="hsla(270, 100%, 50%, {$mouseStrength * ($mousePressed * 0.125 + 0.125)})"
			/>
	...
	{#each boids as boid, i (boid)}
		<circle cx={boid.x} cy={boid.y} r={boid.r} fill="url(#boidGradient{i})" />
	{/each}
